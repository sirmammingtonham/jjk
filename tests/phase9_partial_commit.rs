...
#[test]
fn staged_files_survive_jjk_snapshots_and_scope_the_commit() {
    let (tmp, mut e) = setup();
    ...
    assert!(!e.vcs().snapshot().unwrap().is_empty, "b.txt remains uncommitted");
}

#[test]
fn nothing_staged_commits_everything() {
    let (tmp, mut e) = setup();
    let root = tmp.path().to_path_buf();
    ...
    write(&root, "a.txt", "a\n");
    write(&root, "b.txt", "b\n");
    assert!(e.staged_paths().unwrap().is_empty(), "nothing staged");

    e.commit("everything").unwrap();
    let diff = e.vcs().diff("@-").unwrap();
    assert!(diff.contains("a.txt") && diff.contains("b.txt"), "both files committed: {diff}");
    assert!(e.vcs().snapshot().unwrap().is_empty, "working copy clean after committing all");
}

#[test]
fn explicit_path_scope_leaves_the_rest_behind() {
    let (tmp, mut e) = setup();
    let root = tmp.path().to_path_buf();
    ...
    e.commit_scoped("just keep", &CommitScope::Paths(vec!["keep.txt".into()])).unwrap();

    let diff = e.vcs().diff("@-").unwrap();
    assert!(diff.contains("keep.txt") && !diff.contains("later.txt"), "scoped to keep.txt: {diff}");
    assert!(!e.vcs().snapshot().unwrap().is_empty, "later.txt stays uncommitted");
}
